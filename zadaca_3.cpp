...
public:
    NoCourseException(int index):index(index){}
    void print(){
...
public:
    ...
    virtual int getBodovi(){
        ...
        return (medijana/brojOcenki)*100;
    }
    virtual void pecati(){
        cout<<indeks;
    }
...
public:
    ...
    char const * getCoursename(int i) {return kursevi[i].getIme();}
...
public:
    ...
    Demonstrator(int indeks,int *ocenki,int brojOcenki,char imeIPrezime[],Kurs *kursevi,int brojKursevi,int  brojCasovi): Student(indeks,ocenki,brojOcenki),
                                                                                                                          Predavach(imeIPrezime,kursevi,brojKursevi),brojCasovi(brojCasovi){}
    Demonstrator(const Demonstrator&ref): Student(ref), Predavach(ref),brojCasovi(ref.brojCasovi){}
...
Student& vratiNajdobroRangiran(Student ** studenti, int n ){
    ...
    int max=0;
    for(int i=0;i<n;i++){
        ptr=dynamic_cast<Demonstrator*>(studenti[i]);
        if(ptr==NULL){
            ...
            int bodovi;
            try{
                bodovi=ptr->getBodovi();
            }
            catch(NoCourseException i){
                i.print();
                bodovi=0;
            }
            if(bodovi>max){
...
void pecatiDemonstratoriKurs (char* kurs, Student** studenti, int n){
    ...
    for(int i=0;i<n;i++){
        ptr=dynamic_cast<Demonstrator*>(studenti[i]);
        if(ptr!=NULL){
            for(int j=0;j<ptr->getBrojKursevi();j++){
                if(strcmp(ptr->getCoursename(i),kurs)==0){
                    ptr->pecati();
                    cout<<endl;
                    break;
                }
